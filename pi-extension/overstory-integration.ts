...
async function initializeOverstory(ctx: ExtensionContext): Promise<void> {
  try {
    ...
    logger.debug(`Initializing Overstory. Project Dir: ${projectDir}, Git Exists: ${gitDirExists}, Overstory Dir: ${ovDir}, Overstory Exists: ${ovDirExists}`);
    ...
    if (!ovDirExists) {
        ...
        try {
          ...
          ctx.ui?.notify?.(`Overstory: Created missing .overstory/ directory at ${ovDir}. Run \`ov init\` to complete setup.`, "info");
        } catch (error) {
          logger.debug(`Failed to auto-create directory: ${ovDir}`, error);
          ctx.ui?.notify?.(`Overstory: No .overstory/ directory found at ${ovDir}. Run \`ov init\` to initialize.`, "info");
        }
        return;
    }
